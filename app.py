    ...
    def parse_items(text):
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                items.append({"description": desc.strip(), "price": float(price)})
...
if st.session_state.parsed_items:
    ...
    unassigned_items = [i for i in st.session_state.parsed_items if not any(i in v['items'] for v in st.session_state.assignments.values())]

    selected = []
    for idx, item in enumerate(unassigned_items):
        if st.checkbox(f"{item['description']} - ${item['price']:.2f}", key=f"item_{idx}"):
